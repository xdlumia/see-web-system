...
<template>
  <div class="d-content main-content">
    <div class="mb10">
      <el-input
        v-if="authorityButtons.includes('sys_employee_1004')"
        size="medium"
        @keyup.native.13="$refs.employeeTable.reload()"
        v-model="queryForm.condition"
        placeholder="请输入姓名/电话/员工编号查询"
        class="w240"
      ></el-input>
      <tree-select
        v-if="authorityButtons.includes('sys_employee_1004')"
      ...
      <el-button
        ...
        size="medium"
        type="primary"
        @click="$refs.employeeTable.reload(1)"
        icon="el-icon-search"
      >查询</el-button>
      ...
      >新增用户</auth-button>
      <span
        class="d-inline ml5 d-text-gray"
        v-if="isInMarket&&$refs.employeeTable"
      >员工上限{{totalEmployeeCount||0}}/{{getSourceMaxNum('sys_employee_1001')||'∞'}}</span>
      <div
        class="fr mr10"
        v-if="authorityButtons.includes('sys_employee_1011')"
      >
    ...
    <!-- 表格数据 -->
    <d-table
      v-loading="isTransfering"
      api="bizSystemService.getEmployeeList"
      :params="queryForm"
      v-if="queryForm.sysCode"
      ref="employeeTable"
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span
            v-if="syscode == 'asystem' && scope.row.ransferFlag==1"
            class="d-text-blue d-pointer"
            @click="viewTransferLog(scope.row)"
      ...
      >
        <template slot-scope="scope">
          <el-button
            size="mini"
            v-if="authorityButtons.includes('sys_employee_1009')"
            type="info"
            :disabled="scope.row.userId?true:false"
            plain
            @click="editOrAddHandle('sync',scope.row)"
          >同步用户</el-button>

          <el-button
            v-if="authorityButtons.includes('sys_employee_1007')"
            size="mini"
            type="warning"
            plain
            @click="employeeHandle('employeeAuth',scope.row)"
          >授权</el-button>
          <!-- sourceFrom:   数据来源(0 A系统用户默认方式 1 同步房脉动) -->
          <el-button
            v-if="authorityButtons.includes('sys_employee_1002') && scope.row.sourceFrom!=1"
            size="mini"
            type="primary"
            plain
            @click="editOrAddHandle('edit',scope.row)"
          >修改</el-button>
          <!-- 人员调动功能仅α使用 -->
          <el-button
            v-if="authorityButtons.includes('sys_employee_1010') && scope.row.userId"
            size="mini"
            type="primary"
            plain
            @click="employeeTransfer(scope.row)"
          >人员调动</el-button>
          <el-button
            v-if="authorityButtons.includes('sys_employee_1003') && scope.row.sourceFrom!=1"
            size="mini"
            type="danger"
            @click="delHandle(scope.$index, scope.row)"
          >删除</el-button>
          <el-button
            v-if="authorityButtons.includes('sys_employee_1008') && scope.row.sourceFrom!=1"
            size="mini"
            type="info"
            plain
            @click="editPassWord('password',scope.row)"
          >修改密码</el-button>

          <!--增加 锁定用户、解锁用户功能-->
          <el-button
            v-if="authorityButtons.includes('sys_employee_1005') && scope.row.lockStatus==0 && scope.row.sourceFrom!=1"
            size="mini"
            type="warning"
            plain
            @click="lockUser('lock',scope.row)"
            title="锁定用户"
          >锁定</el-button>
          <el-button
            v-if="authorityButtons.includes('sys_employee_1006') && scope.row.lockStatus==1 && scope.row.sourceFrom!=1"
            size="mini"
            type="warning"
            plain
            @click="unLockUser('unlock',scope.row)"
            title="解锁用户"
          >解锁</el-button>
          <!--end -->
      ...
      <div v-loading="loading">
        ...
        >
          <div v-if="dialogType=='add' || dialogType=='edit'">
            ...
            >
              <el-input
                v-model.trim="dialogForm.employeeNo"
                placeholder="请输入员工编号"
                maxlength="25"
                class="w200"
              ></el-input>
            ...
            >
              <el-input
                v-model.trim="dialogForm.positionName"
                placeholder="请输入职位名称"
                :maxlength="25"
                class="w200"
              ></el-input>
            ...
            >
              <el-input
                ...
                placeholder="请输入电话"
                class="w200"
                :disabled="!!dialogForm.userAccount"
                :maxlength="11"
                :minlength="11"
              ></el-input>
            </el-form-item>
            ...
            >
              <upload-file
                :limit="{size:'5M',type:['png','jpg','gif','bmp']}"
                @addPictureUrl="setAvatar"
              >
                <img
        ...
        <div v-if="dialogType=='sync'">
          <span>密码:</span>
          <el-input
            v-model="syncForm.pwd"
            size="small"
            :disabled=rowData.registerPassword
            placeholder="请输入密码"
            :type="asyncType"
            class="w200"
          >
            <i
              ...
              title="显示密码"
              :class="{'d-text-blue': asyncType == 'text' ,'d-text-blue':asyncType == 'password'}"
            ></i>
          </el-input>
        </div>
      </div>
      ...
      >
        ...
        <el-form-item>
          <el-button
            size="small"
            type="primary"
            @click="updateSaveHandle('passwordform')"
          >确定</el-button>
          <el-button
            size="small"
      ...
      >
        ...
        <el-form-item>
          <el-button
            size="small"
            type="primary"
            @click="lockUserFunc('lockform')"
          >确定</el-button>
          <el-button
            size="small"
            @click="dialogVisbleLock =false"
          >取消</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
