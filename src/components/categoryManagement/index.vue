...
<template>
  <!-- 类目管理 -->
  <div class="cate" v-loading="loading" style="min-width:1256px;">
    <div class="cate-top">
      <el-button
    ...
    <div class="jianTree" style="overflow:auto; height:calc(100% - 68px)">
    ...
    <div>
      <el-pagination
        @size-change="fgoodsFirstClassList"
        @current-change="curChangeHandle"
        :current-page.sync="firstForm.page"
        :page-size.sync="firstForm.limit"
...
export default {
  ...
  methods: {
    fgoodsFirstClassList() {
      this.defalutOpenArr = [];
      this.loading = true;
      this.$api.seeGoodsService
        .fgoodsFirstClassList(this.firstForm) // 获取物品一级类目
        .then(res => {
          let data = res.data || [];
    ...
    lazyTreeData(node, resovle) {
      ...
      // console.log(resovle)
      this.$api.seeGoodsService
        .fgetChildClassList({ id: node.data.id }) // 获取物品子类类目
        .then(res => {
    ...
    fcateChange(node, data) {
      ...
      if (data.isEnable == 1) {
        ...
        this.loading = true;
        this.$api.seeGoodsService
          .handelGoodsChild({ id: data.id, isEnable: data.isEnable }) //
          .then(res => {
            this.loading = true;
            if (this.treeClickData.parentId) {
              this.fgoodsFirstClassList();
              this.$api.seeGoodsService
                .fgetChildClassList({ id: this.treeClickData.parentId }) // 获取物品子类类目
                .then(res => {
        ...
        this.$confirm(
          "分类设为无效后，选择或查询物品时，将不会再看到无效分类，您确认要把 " +
            data.className +
            " 分类设为无效么？",
          "提示",
          {
    ...
    fcatetop(data, node) {
      ...
      this.treeClickData = data;
      this.$api.seeGoodsService
        .handelGoodsChild({ id: data.id, isTop: 1 }) //
        .then(res => {
          this.loading = true;
          if (this.treeClickData.parentId) {
            this.$api.seeGoodsService
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (!this.handel) {
            // 编辑模式下，走修改接口
            if (this.istype === "itself") {
              // 点击的是编辑n
              this.$api.seeGoodsService
                .handelGoodsChild({
                  className: this.form.handelClassName,
                  id: this.treeId
                }) //
                .then(res => {
                  this.loading = true;
            ...
            } else if (this.istype === "child") {
              // 点击的是新增子类
              this.handel = false;
              this.addchild = true;
              this.$confirm(
                "分类新增成功后，不可更改及删除，请仔细核实分类信息！您新增的子类名称为 " +
                  this.form.handelClassName +
                  " ,所属分类为 " +
                  this.treeClickData.className +
                  " ，是否确认无误？",
                "提示",
                {
                  confirmButtonText: "确定",
                ...
                .then(() => {
                  this.$api.seeGoodsService
                    ...
                    .then(res => {
                      this.loading = true;
                      this.$api.seeGoodsService
                        .fgetChildClassList({ id: this.treeClickData.id }) // 获取物品子类类目
                        .then(res => {
                          this.loading = true;
                          for (let i = 0; i < res.data.length; i++) {
                            res.data[i].isEnable = res.data[
            ...
            // 新增
            this.$confirm(
              "分类新增成功后，不可更改及删除，请仔细核实分类信息！您新增的分类名称为 " +
                this.form.handelClassName +
                " ，是否确认无误？",
              "提示",
              {
