  ...
  <div class="process ml20" v-loading="showLoading">
    ...
    <el-row class="h60 pl20 title" type="flex" justify="space-between" align="middle">
      ...
      <el-col :span="18" class="ar">
        <div class="d-inline d-text-gray" style="font-weight:400">
          [自定义]审核流程节点责任人
          <el-tooltip class="item" effect="dark" placement="top">
            <div slot="content" v-html="userProcessAuditor.tips"></div>
            <i class="d-inline el-icon-info"></i>
          </el-tooltip>
          <el-switch class="mr20" @change="updateProcessSet" v-model="userProcessAuditor.switch"></el-switch>
        </div>
    ...
    <div class="mb20">
      <el-row
        v-for="(mainItem, mainIndex) in dataList"
        :key="mainIndex"
        style="position: relative;"
      >
        <el-row class="h40 mt20 pl20 title">{{mainItem.processName}}</el-row>
        <div
          class="line"
          style="width: 94%;height: 3px;background-color: #BCBCBC;position: absolute;top: 57%;left: 3%;"
        ></div>
        <el-row style="border: 1px solid #d3dce6;border-radius: 2px;" type="flex">
          <draggable
            @update="dragUpdate(mainItem.content.mainTask.subTasks)"
            :options="{ animation:150,draggable:'.dragHandle', scroll:true,scrollSensitivity:200}"
            class="d-flex wfull"
            v-model="mainItem.content.mainTask.subTasks"
          >
            <el-col
              :class="{dragHandle:subItem.taskCode!='START'&&subItem.taskCode!='END'}"
              v-for="(subItem, subIndex) in mainItem.content.mainTask.subTasks"
              :key="subIndex"
              style="display: flex;height: 200px;justify-content: center;align-items: center;flex-direction: column;user-select: none;"
            >
              <div style="background-color: #fff;">
                <i
                  class="iconfont"
                  :class="[iconList[subItem.taskCode], subItem.taskStatus === 'default' ?  'default' : ( subItem.taskStatus === 'enable' ? 'enable' : 'disable' )]"
                  style="font-size: 60px;cursor: pointer;"
                  @click="changeTaskStatus(subIndex, mainIndex)"
                ></i>
              </div>
...
export default {
  ...
  data () {
    return {
      ...
      iconList: {
        ...
        asystem_contract_bargain_1023: 'icon-caiwudaodan', // 商业集中-合同变更审核流程 财务主管审核
        asystem_contract_bargain_1022: 'icon-baohu', // 商业集中-合同变更审核流程 运营总监审核
        asystem_contract_bargain_1024: 'icon-zhang' // 商业集中-合同变更审核流程 行政专业审核
      }
    }
  },
  mounted () {
  ...
  methods: {
    ...
    saveProcessDefinition () {
      this.showLoading = true
      let processDefinitionUpdateVos = this.dataList.map(function (
        element,
        index
      ) {
      ...
      this.$api.seeHouseConfigService
        .saveProcessDefinition(updateArrayVoVo)
        .then(res => {})
        .finally(() => {
          let timer = setTimeout(() => {
            this.showLoading = false
    ...
    changeTaskStatus (subIndex, mainIndex) {
      ...
      if (_dataArray[subIndex].taskStatus === "default") return;
      else if (
        _dataArray[subIndex].taskStatus != "disable" &&
        _dataArray[subIndex].taskStatus != "enable"
      ) {
        console.log(
          "taskStatus属性值错误！taskStatus属性只能是disable或者enable"
        );
        
      } else {
        _dataArray[subIndex].taskStatus =
          _dataArray[subIndex].taskStatus === "enable" ? "disable" : "enable";
      }
    },
    /**
    ...
    queryProcessSet () {
      this.$api.seeHouseConfigService
        .queryProcessSet('ifUserDefineProcessAuditor')
        .then(res => {
    ...
    updateProcessSet () {
      ...
      this.$api.seeHouseConfigService
        .updateProcessSet(this.userProcessForm)
        .then(res => {
          // console.log('更改成功');
        })
    }
